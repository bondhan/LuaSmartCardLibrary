...
static const struct luaL_Reg pcsc_cardlib[] =
{
	{ "list_readers", lua_listreaders },
	{ "init_reader", bool_ret_function },
	{ "reset_and_sleep", bool_ret_function },
	{ "connect_reader", lua_connect_reader },
	{ "disconnect_reader", lua_disconnect_reader },
	{ "get_atr", lua_get_atr },
	{ "send", lua_send_apdu},
	{ "send_auto", lua_send_apdu_auto},
	{ "do_pps", lua_dopps },
	{ "stringify_sw", subr_card_stringify_sw },
	{ "warm_reset", subr_card_warm_reset },
	{ "cold_reset", lua_cold_reset},
	{ "set_voltage", bool_ret_function },
	{ "set_timeout", one_param_bool_ret_function },
	{ "get_card_info", lua_get_card_info },
	{ NULL, NULL }  /* sentinel */
};
...
{
	luaL_newlib(L, pcsc_cardlib);
	lua_setglobal(L, "pcsc_card");
...
int lua_send_apdu(lua_State *L)
{
	...
	unsigned short SW = send_apdu(L, apdu_bytestring, result, 1);
...
int lua_send_apdu_auto(lua_State *L)
{
	...
	SW = send_apdu(L, apdu_bytestring, result, 1);
...
int send_apdu(lua_State *L, const bytestring_t *command, bytestring_t *result, int getResponse)
{	
	...
	tmp = bytestring_to_format("%D", command);
	log_printf(LOG_INFO, "TERM->CARD: %s [%s]", tmp, iso7816_stringify_apdu_class(ad.apdu_class));		
	free(tmp);
	...
	log_printf(LOG_INFO, "CARD->TERM: %04X %s ", SW, tmp);
	free(tmp);
	...
	if (getResponse == 1)
	{
		if (SW1 == 0x6C) /* Re-issue with right length */
		{
			...
			SW = send_apdu(L, command_dup, result, getResponse);
		...
		else
		{
			while (SW1 == 0x61) /* use Get Response */
			{
				...
				SW = send_apdu(L, get_response, tmp_response, getResponse);
