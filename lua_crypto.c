...
static int subr_crypto_mac_iv(lua_State* L)
{
  bytestring_t* ctx = luaL_check_bytestring(L,1);
  bytestring_t* src = luaL_check_bytestring(L,2);
  bytestring_t* iv = luaL_check_bytestring(L,3);
  bytestring_t* res = bytestring_new(8);
  crypto_error_t e;

  if ((e=crypto_mac_iv(res,ctx,src,iv))==CRYPTO_OK)
    lua_push_bytestring(L,res);
  else
  {
    bytestring_free(res);
